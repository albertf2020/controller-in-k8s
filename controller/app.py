...
class KubernetesClient:
    ...
    """
    Класс для взаимодействия с Kubernetes API.
    ...
    def __init__(self):
        ...
        """
        Инициализация объекта KubernetesClient.
        Устанавливает флаг verify_cert на основе переменной окружения VERIFY_CERT.
    ...
    def get_auth_headers(self):
        ...
        """
        Возвращает заголовки аутентификации с Bearer Token.
        Сначала пытается получить токен из переменной окружения K8S_BEARER_TOKEN.
        Если токен не найден, читает токен из файла /var/run/secrets/kubernetes.io/serviceaccount/token.

        Возвращает:
            dict: Словарь с заголовками аутентификации.
        """
...
class KubeStateMetricsClient(KubernetesClient):
    ...
    """
    Клиент для получения и парсинга данных из Kube State Metrics.
    Наследуется от класса KubernetesClient для использования функционала аутентификации.
    ...
    def __init__(self):
        ...
        """
        Инициализирует объект KubeStateMetricsClient.
        Вызывает инициализацию родительского класса KubernetesClient.
        Устанавливает URL Kube State Metrics из переменной окружения KUBE_STATE_METRICS_URL.
        Если переменная не установлена, выводится предупреждение, и URL не инициализируется.
        ...
        if not self.kube_state_url:
            logging.warning("Переменная окружения 'KUBE_STATE_METRICS_URL' не установлена; невозможно получить запросы/лимиты.")

    def fetch_metrics(self):
        
        """
        Делает запрос к Kube State Metrics и возвращает словарь с ресурсами (запросы/лимиты) для каждого контейнера.

        Возвращает:
            dict: Словарь с полученными ресурсами или пустой словарь, если URL не установлен или возникла ошибка.
        """
        ...
        try:
            ...
            logging.error(f"Ошибка при запросе к Kube State Metrics: {e}")
            return {}

        data = self.parse_metrics(resp.text)

        if not data and logging.getLogger().isEnabledFor(logging.DEBUG):
            logging.debug("Нет данных ресурсов, полученных из Kube State Metrics")
    ...
    def parse_metrics(self, text):
        ...
        """
        Парсит текстовые метрики в формате Prometheus из Kube State Metrics.

        Аргументы:
            text (str): Текстовые данные в формате Prometheus, полученные от Kube State Metrics.

        Возвращает:
            dict: Словарь с ключами в виде кортежей (namespace, pod, container) и значениями в виде словарей
                  с распарсенными ресурсами (запросы/лимиты) для каждого контейнера.
        """

        # Регулярные выражения для сопоставления реального формата метрик
...
class MetricsServerClient(KubernetesClient):
    ...
    """
    Клиент для извлечения данных с Metrics Server
    ...
    def fetch_metrics(self):
        ...
        """
        Делает запрос к Metrics Server и возвращает метрики использования памяти/ЦП
    ...
    def parse_pod_metrics(self, pod_metrics, state_data):
        ...
        """
        Анализирует метрики модуля и возвращает словарь с данными об использовании
        {
...
class KubernetesApiClient(KubernetesClient):
    ...
    def patch_pod_resources(self, namespace, pod_name, container_name, memory_request, memory_limit=None):
        ...
        """
        Обновляет ресурсы pod с помощью API Kubernetes с InPlacePodVerticalScaling
    ...
    def update_last_scale_annotation(self, namespace, pod_name):
        ...
        """
        Обновляет под с аннотацией, отслеживающей время последнего масштабирования.
...
class ResourceManager:
    ...
    Менеджер который контролирует и регулирует ресурсы модуля на основе фактического использования
    ...
    def run(self):
        ...
        """
        Основной цикл для мониторинга и управления ресурсами
    ...
    def _process_resources(self):
        ...
        """
        Выполнение для всех подов, соответствующих селектору меток
    ...
    def _evaluate_and_scale(self, namespace, pod_name, container_name, metrics):
        ...
        """
        Оценивает, нуждается ли контейнер в масштабировании, и выполняет масштабирование при необходимости
    ...
    def _scale_up(self, namespace, pod_name, container_name, memory_usage, current_request):
        ...
        """
        Масштабирует ресурсы контейнера для предотвращения OOM
    ...
    def _scale_down(self, namespace, pod_name, container_name, memory_usage, current_request):
        ...
        """
        Уменьшает ресурсы контейнера, если их использование было низким в течение достаточного времени
